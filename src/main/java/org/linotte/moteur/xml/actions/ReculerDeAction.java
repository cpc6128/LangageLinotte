...
public class ReculerDeAction extends Action implements IProduitCartesien {

	public ReculerDeAction() {
		super();
	}

	@Override
	public String clef() {
		return "verbe reculer de";
	}

	@Override
	public ETAT analyse(String param, Job job, ItemXML[] valeurs, String[] annotations) throws Exception {

		Acteur[] acteurs = extractionDesActeurs(Constantes.SYNTAXE_PARAMETRE_RECULER, job, valeurs);
		Acteur acteur1 = acteurs[0];
		Acteur acteur2 = acteurs[1];

		if (!(acteur2.getRole() == Role.NOMBRE) && !(acteur1.getRole() == Role.ESPECE))
			throw new ErreurException(Constantes.SYNTAXE_PARAMETRE_RECULER);

		Prototype e = (Prototype) acteur1;
		if (!e.isEspeceGraphique())
			throw new ErreurException(Constantes.SYNTAXE_PARAMETRE_RECULER, e.toString());
		...
		BigDecimal x2 = x.subtract(c.multiply(new BigDecimal(Math.cos(angle)), Mathematiques.getPrecision()));
		BigDecimal y2 = y.subtract(c.multiply(new BigDecimal(Math.sin(angle)), Mathematiques.getPrecision()));
