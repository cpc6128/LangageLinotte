...
public class DeplacerVersGaucheAction extends Action implements IProduitCartesien {

	public DeplacerVersGaucheAction() {
		super();
	}

	@Override
	public String clef() {
		return "verbe déplacer vers gauche";
	}
	...
	public ETAT analyse(String param, Job job, ItemXML[] valeurs, String[] annotations) throws Exception {
		...
		if (eg.retourneAttribut("x") != null) {
			// Crayon ?
			BigDecimal x = ((BigDecimal) eg.retourneAttribut("x").getValeur()).subtract(((BigDecimal) acteur2.getValeur()));
		...
		if (eg.retourneAttribut("x1") != null)
			eg.retourneAttribut("x1").setValeur(((BigDecimal) eg.retourneAttribut("x1").getValeur()).subtract(((BigDecimal) acteur2.getValeur())));
		if (eg.retourneAttribut("x2") != null)
			eg.retourneAttribut("x2").setValeur(((BigDecimal) eg.retourneAttribut("x2").getValeur()).subtract(((BigDecimal) acteur2.getValeur())));
