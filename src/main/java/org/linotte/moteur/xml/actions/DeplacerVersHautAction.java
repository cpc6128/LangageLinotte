...
public class DeplacerVersHautAction extends Action implements IProduitCartesien {

	public DeplacerVersHautAction() {
		super();
	}

	@Override
	public String clef() {
		return "verbe déplacer vers haut";
	}
	...
	public ETAT analyse(String param, Job job, ItemXML[] valeurs, String[] annotations) throws Exception {
		...
		if (eg.retourneAttribut("x") != null) {
			...
			BigDecimal y = ((BigDecimal) eg.retourneAttribut("y").getValeur()).subtract(((BigDecimal) acteur2.getValeur()));
		...
		if (eg.retourneAttribut("x1") != null)
			eg.retourneAttribut("y1").setValeur(((BigDecimal) eg.retourneAttribut("y1").getValeur()).subtract(((BigDecimal) acteur2.getValeur())));
		if (eg.retourneAttribut("x2") != null)
			eg.retourneAttribut("y2").setValeur(((BigDecimal) eg.retourneAttribut("y2").getValeur()).subtract(((BigDecimal) acteur2.getValeur())));
