...
public class LibrairieVirtuelle implements Librairie<LibrairieVirtuelle> {

	private Map<Chaine, Acteur> librairie;

	public Map<Chaine, Acteur> acteurs_pour_espece_map = new Hashtable<Chaine, Acteur>(100);

	public Map<String, Acteur> espece_map = new Hashtable<String, Acteur>(100);

	private LaToile toilePrincipale;

	private Map<String, LaToile> toiles = new Hashtable<String, LaToile>(1);

	public LibrairieVirtuelle(String cf) {
		librairie = new ConcurrentHashMap<Chaine, Acteur>(10000);
	}
	...
	public void mettre(Acteur a) throws ErreurException {
		...
		} else if (a.getNom() != null && a.getNom().length() != 0) {
	...
	public void supprime(Acteur a) throws ErreurException {
		if (a.getNom() != null && a.getNom().length() != 0) {
	...
	public void reCharger(Acteur a) throws ErreurException {
		...
		if (a.retourneFichier() != null) {
			...
			Acteur temp = (Acteur) librairie.get(a.getNom());
			// Patch correction bogue avec les espèces et des attributs
			// différents:
			if (temp != null && a != temp && temp.getRole() == a.getRole() && a.equalsActeur(temp)) {
				a.copier(temp);
			} else {
				if (temp != null && temp.getRole() != a.getRole()) {
					throw new ErreurException(Constantes.SYNTAXE_ACTEUR_DEJA_PRESENT_AVEC_ROLE_DIFF, a.getNom().toString());
				}
			}
		}
	}
	...
	public void setToile(LaToile latoile) {
		...
		if (latoile != null) {
			toiles.put("toileprincipale__", latoile);
		}
	}
	...
	@Override
	public LibrairieVirtuelle cloneMoi() {
		LibrairieVirtuelle copie = new LibrairieVirtuelle("copie");
		copie.espece_map = espece_map;
