...
public class DeplacerDeAction extends Action implements IProduitCartesien {

	public DeplacerDeAction() {
		super();
	}

	@Override
	public String clef() {
		return "verbe déplacer de";
	}
	...
	public ETAT analyse(String param, Job job, ItemXML[] valeurs, String[] annotations) throws Exception {
		...
		if (eg.retourneAttribut("x") != null) {
			// Crayon ?
			BigDecimal x = ((BigDecimal) eg.retourneAttribut("x").getValeur()).add((BigDecimal) acteur2.getValeur());
			BigDecimal y = ((BigDecimal) eg.retourneAttribut("y").getValeur()).add((BigDecimal) acteur3.getValeur());
			...
			eg.retourneAttribut("x").setValeur(x);
			eg.retourneAttribut("y").setValeur(y);
		}
		if (eg.retourneAttribut("x1") != null) {
			eg.retourneAttribut("x1").setValeur(((BigDecimal) eg.retourneAttribut("x1").getValeur()).add((BigDecimal) acteur2.getValeur()));
			eg.retourneAttribut("y1").setValeur(((BigDecimal) eg.retourneAttribut("y1").getValeur()).add((BigDecimal) acteur3.getValeur()));
		}
		if (eg.retourneAttribut("x2") != null) {
			eg.retourneAttribut("x2").setValeur(((BigDecimal) eg.retourneAttribut("x2").getValeur()).add((BigDecimal) acteur2.getValeur()));
			eg.retourneAttribut("y2").setValeur(((BigDecimal) eg.retourneAttribut("y2").getValeur()).add((BigDecimal) acteur3.getValeur()));
		}

		// LaToile.getThis().repaint();
