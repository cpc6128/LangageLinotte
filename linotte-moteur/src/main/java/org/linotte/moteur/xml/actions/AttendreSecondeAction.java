...
public class AttendreSecondeAction extends Action implements IProduitCartesien {

	public AttendreSecondeAction() {
		super();
	}

	private static BigDecimal mille = new BigDecimal(1000);

	@Override
	public String clef() {
		return "verbe attendre seconde";
	}
	...
	public ETAT analyse(String param, Job runtime, ItemXML[] valeurs, String[] annotations) throws Exception {
		...
		try {
			long valeur = ((BigDecimal) a.getValeur()).multiply(mille).longValue();
			if (valeur > 0)
				Thread.sleep(valeur);
