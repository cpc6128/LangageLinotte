...
public class Diff extends OperationBinaire {
	...
	public Object calculer(Job moteur, Temoin temoin, Object token_courant) throws Exception {
		...
		if (o1 instanceof Prototype) {
			...
			throw new Exception("Opération mathématique 'diff' interdite avec " + o1);
		}
		...
		if (o2 instanceof Prototype) {
			...
			throw new Exception("Opération mathématique 'diff' interdite avec " + o2);
		}

		if (operationPrototype) {
			if (!(bd1 instanceof Prototype))
				throw new Exception("Opération mathématique 'diff' interdite avec " + bd1);
			if (!(bd2 instanceof Prototype))
				throw new Exception("Opération mathématique 'diff' interdite avec " + bd2);
			return (ConditionAction.equals((Acteur) bd1, (Acteur) bd2) ? MathematiquesConstantes.FAUX : MathematiquesConstantes.VRAI);
		} else if (operationChaine)
			return (((String) bd1).equalsIgnoreCase((String) bd2) ? MathematiquesConstantes.FAUX : MathematiquesConstantes.VRAI);
		else
